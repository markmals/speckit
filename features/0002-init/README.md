...
Project bootstrapping and the per-agent command projection (D2/D4), plus the additive extension mechanism that replaces Workbench's superset-then-prune `/setup` (D3). These specs gate **Phase 2** (`init`, projection) and **Phase 4** (extension round-trip).
