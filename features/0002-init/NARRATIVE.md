...
Unlike upstream spec-kit — where `specify` is an installer that disappears — the SpecKit binary stays. `init` is just one of its subcommands, the same binary that later runs `scan`, `verify`, and `drift`. Choosing platforms is additive, not subtractive: `--platforms web,apple` installs the corresponding first-party extension packs rather than pruning a superset. Adding a platform later is `extension add`, and removing it restores the prior state cleanly.
