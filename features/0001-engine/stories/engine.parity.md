...
# Story: Parity across platforms

As a developer or agent maintaining N native implementations of one spec,
I want `specify parity` to show, per (platform × scenario), an honest classification of conformance,
...
- Given a platform whose joined test for a scenario passes and whose source carries no `(deviates:)` marker for it
...
- Given a platform whose joined test passes and whose source carries a `(deviates: <reason>)` marker for that scenario
- When the user runs `specify parity`
...
- Given a platform whose joined test for a scenario FAILS but whose source carries a `(deviates:)` marker for it
- When the user runs `specify parity`
...
- Given a platform whose joined test fails and whose source carries no deviation marker
- When the user runs `specify parity`
...
- Given a platform that neither tests nor carries a reverse pointer for a scenario's spec
- When the user runs `specify parity`
...
- Given any platform/scenario pair
