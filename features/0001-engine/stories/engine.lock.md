...
So that drift state is recorded deterministically and exactly once, only when a target is genuinely green (D7).
...
- Given `specify verify <target>` passes every scenario of a spec
- Then it invokes `specify lock` to write `.speckit/lock/<target>/<spec-id>` with the spec's content hash and per-scenario results
...
- Given `specify verify <target>` has any failing or unjoinable scenario
