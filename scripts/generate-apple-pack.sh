...
Regenerates the embedded SpecKit apple stack pack from the mac-dev-skills
appkit plugin source. The iOS skills in the pack are left in place; the AppKit
skills and appkit-dev stack agent are generated.

Options:
  --check        regenerate in a temp dir and fail if the committed pack drifts
