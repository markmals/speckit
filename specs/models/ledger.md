...
The append-only record of regeneration attempts — the raw material for `specify bench` (framework-curation receipts) and for understanding agent throughput per spec/platform.

## Shape

Append-only JSONL under `.speckit/`; one record per `apply` attempt:

- `spec`, `platform` — what was applied where.
...
- **G3 — Self-describing.** A record carries enough to reconstruct the attempt's outcome without external state (spec, platform, results, timings).
