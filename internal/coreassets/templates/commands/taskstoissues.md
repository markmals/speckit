---
description: File a feature's target task list as GitHub issues on the repo matching the git remote.
---

# /speckit.taskstoissues — Tasks → GitHub issues

Turn `features/<NNNN>-<slug>/tasks/<target>.md` into GitHub issues so the work is trackable in the same place SpecKit's git workflow already lives (worktrees, PRs, Projects — see the README).
...
2. **Read** `features/<NNNN>-<slug>/tasks/<target>.md`. Each task line becomes one issue.
3. **Create**, one issue per task, with `gh issue create`:
   - Title: `<spec-id> — <task summary> (<target>)`.
   - Body: the task detail, the file path, and a link to the spec file it realizes.
   - Labels: the target and the owning story (`US#`) where the repo has them; create labels only if the user approves.
