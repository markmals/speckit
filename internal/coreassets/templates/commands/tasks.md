...
**Arguments:** `<feature> <target>` — e.g. `/speckit.tasks 0001-managing-items ios`.

## Workflow

Read the feature folder and `plans/<target>.md` (if present). Write `features/<NNNN>-<slug>/tasks/<target>.md` as a checklist, phased:

- **Phase 1 — Setup:** project/test scaffolding this target needs.
...
→ `/speckit.implement <feature> <target>` (follows these phases), or `/speckit.taskstoissues <feature> <target>` to file them as GitHub issues. Commit prefix `plan:`/`<target>:`, scoped per `specify gate scope`.
