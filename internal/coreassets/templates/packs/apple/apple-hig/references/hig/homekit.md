...
In the HomeKit model, the [home](homekit.md#Homes) object is the root of a hierarchy that contains all other objects, such as [rooms](homekit.md#Rooms), [accessories](homekit.md#Accessories-services-and-characteristics), and [zones](homekit.md#Zones). When there’s more than one home, each home is the root of a different hierarchy.
...
**Pair the icon with the name _HomeKit_ correctly.** You can show the name below or beside the icon if other technologies are referenced in this way. Use the same font that’s used on the rest of your layout. For related guidance, see [Referring to HomeKit](homekit.md#Referring-to-HomeKit).
