...
Starting in watchOS 9, the system organizes complications (also known as _accessories_) into several families — like [circular](complications.md#Circular) and [inline](complications.md#Inline) — and defines some recommended layouts you can use to display your complication data. A watch face can specify the family it supports in each complication slot. Complications that work in earlier versions of watchOS can use the [legacy templates](complications.md#Legacy-templates), which define nongraphic complication styles that don’t take on a wearer’s selected color.
