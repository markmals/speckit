...
type Adapter interface {
	...
	// concept.
	SkillsDir() string
	// AgentsDir is the directory (relative to root, slash-separated) where the
	// review subagents from init are written, or "" if the agent has no
	// subagent/agent-file concept. Today dispatched subagents are a Claude Code
	// feature (the claude-pack).
	AgentsDir() string
